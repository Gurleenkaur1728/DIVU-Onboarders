...
-- Create RLS policies
CREATE POLICY "Users can insert their own feedback" ON module_feedback
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can view their own feedback" ON module_feedback
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can update their own feedback" ON module_feedback
