...
export default function Login() {
 ...
 const handleLogin = async (e) => {
   ...
   try {
     setLoading(true);
     // 1) Get user profile directly from users table
     ...
     // 3) Role from user_roles (fallback to users.role_id)
     let rid = 0;
     let rtxt = "";
     ...
     const isAdmin =
       rid === 1 || rid === 2 || rtxt === "admin" || rtxt === "superadmin";
     // 4) Persist session
     ...
     localStorage.setItem(
       "role",
       rtxt || (isAdmin ? (rid === 2 ? "superadmin" : "admin") : "user")
     );
     setLoading(false);
     // 5) Redirect
